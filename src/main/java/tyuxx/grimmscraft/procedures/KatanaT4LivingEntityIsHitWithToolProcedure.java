...
public class KatanaT4LivingEntityIsHitWithToolProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("xp") >= itemstack.getOrCreateTag().getDouble("xpn")) {
			...
			itemstack.getOrCreateTag().putDouble("xpn", Math.ceil(itemstack.getOrCreateTag().getDouble("xpn") * (GrimmscraftModVariables.MapVariables.get(world).katanalevelupdif - 0.05)));
